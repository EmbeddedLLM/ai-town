...
import { dumpConversationTable } from './conversation';
...
/**
 * All implementations regarding to an agent's memory should be placed in here. Usually post conversation actions.
 * 
 * @author wenjie
 */

/**
 * This function is called by both agents after every conversation is done. It will first request the LLM
 * for a summary of the conversation, then it would check if the conversation has exceeded a specified
 * token threshold. If it exceeds, it would get all chat rows and dump them into the knowledge table of the agent,
 * to act as a memory repository. 
 * 
 * @author wenjie
 * @param ctx 
 * @param worldId 
...
export async function rememberConversation(
  ...
  const llmMessages: LLMMessage[] = [
    {
      ...
      'I, and add if you liked or disliked this interaction. Refer back to your chat history for this interaction.`,
    },
  ];
  let count = 0;
  ...
  console.log(`Remembering convo for ${player.name}: tokens: ${content.getTotalTokens()}`)

  // Dump the conversation into knowledge table to act as an agent's memory repository. 
  if (content.getTotalTokens() >= 1000) {
    await dumpConversationTable(
      ctx,
      worldId,
      conversationId,
      playerId as GameId<'players'>,
      otherPlayer.id as GameId<'players'>,
    )
  }

  // if (content.getTotalTokens() >= 1000) {
  //   // Delete the conversation table to maintain context length of agent
  //   await jamAiApi.deleteConversationTable(`${player.name}-${otherPlayer.name}`)
  //   await jamAiApi.deleteConversationTable(`${otherPlayer.name}-${player.name}`)

  // }

  // This is to store a local copy into the sqlite db. Removing this causes messages to stop appearing
  // in chat. idek why. 
...
async function reflectOnMemories(
  ...
  const { content: reflection } = await chatCompletion({
    ...
    max_tokens: 300,
    stream: false,
    conversationId: `Chat_Ai-Town-${name}`
  });
