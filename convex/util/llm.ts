import * as api from "../aiTown/JamAIBaseAPi"

/**
 * All chat completions and generative actions go through here, and should be placed here.  
 * @author wenjie
 */


/**
 * This config is irrelevant because we are using Jamaiapi.
 * 
 * @author wenjie
 */
export const LLM_CONFIG = {
   ...
   */
  ollama: true,
  url: 'http://192.168.80.80:8000',
  chatModel: 'meta-llama/Llama-3-8B-Instruct' as const,
...
): Promise<{ content: string; tokens: number; retries: number; ms: number }>;
...
try{
    // Cleaning up the request body to ensure there are no nested quotes which were causing body errors.
    const contentStrings = body.messages.map(message => message.content).join('\n').replace(/^["']+|["']+$/g, '');
    const result = await api.generateTextDuringInteraction(body.conversationId, contentStrings, undefined, body.stream as any)
...
/**
 * Locally, the game requests for embedding to be stored in the sqlite db for some magical reason.
 * It breaks if i take this way, despite everything already migrated to jamai. TODO: use jamai embedding
 * endpoint when it gets implemented. 
 * 
 * @author wenjie
 * @param texts 
 * @returns 
...
export async function fetchEmbeddingBatch(texts: string[]) {
  ...
  const {
    ...
    const result = await fetch("https://api.jamaibase.com/api/v1/embeddings", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export interface CreateChatCompletionRequest {
  ...
  n?: number | null;
   /**
   * convoId to interact with JamAi conversation agent
   * @type {string}
   * @memberof CreateChatCompletionRequest
   */
...
/**
 * Edited to track token count for memory dumps. 
 */
export class ChatCompletionContent {
  ...
  async *splitStream(stream: any) {
    // console.log("after result3");
    // console.log(stream)
    const reader = stream.getReader();
    // console.log("after result4");
    let lastFragment = '';
    try {
        while (true) {
            // console.log("after result5");
            const { value, done } = await reader.read();
            // console.log("after result6");
            if (done) {
...
export async function ollamaFetchEmbedding(text: string) {
  const { result } = await retryWithBackoff(async () => {
     ...
     const resp = await fetch("https://cloud.jamaibase.com/api/v1/embeddings", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.JAMAI_API_KEY}`,
